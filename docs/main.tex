...
During training, the DiceFocalLoss composite loss function is adopted to alleviate class imbalance, combined with the AdamW optimizer and cosine annealing learning rate scheduling. Three experiments were conducted on the Kaggle Chest CT Segmentation dataset: Experiment~I (Swin-UNETR~v2 baseline), Experiment~II (Swin-UNETR~v2 with parameter optimization verification), and Experiment~III (UNet++ with pretrained encoder). The best model from Experiment~III achieves a mean Dice coefficient of 0.9661 on the test set, with per-organ Dice scores of 0.9702 for lung, 0.9718 for heart, and 0.9563 for trachea. A reference baseline using U-Net with EfficientNet-B2 encoder and BCEDiceLoss achieved val\_dice of 0.968 on a different data split.

Experimental results demonstrate that the UNet++ with pretrained encoder effectively captures semantic features of organs at varying scales in chest CT images, achieving the best performance on the multi-organ segmentation task. This report provides comprehensive details on the methodology, implementation, training dynamics, and results analysis, along with complete experimental records and visualization examples.
...
训练阶段采用DiceFocalLoss复合损失函数缓解类别不平衡问题，配合AdamW优化器与余弦退火学习率调度策略，在Kaggle Chest CT Segmentation数据集上进行了三组实验：实验一（Swin-UNETR~v2基准模型）、实验二（Swin-UNETR~v2参数优化验证）和实验三（基于预训练UNet++的对比实验）。实验三的最优模型在测试集上取得了0.9661的平均Dice系数（mDSC），肺部分割Dice系数最高达0.9702，心脏0.9718，气管0.9563。此外，基于Kaggle开源示例的参考基线（U-Net + EfficientNet-B2编码器 + BCEDiceLoss）在不同数据划分上取得了val\_dice=0.968的结果。

实验结果表明，采用预训练编码器的UNet++能够有效捕获胸部CT图像中不同尺度器官的语义特征，在多器官分割任务上取得了最优性能。本报告详细阐述了研究方法、实现过程、训练动态和结果分析，提供了完整的实验记录和可视化示例。
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
    ...
    \item 通过三组独立实验验证方法的有效性与参数优化的可复现性，最优模型（UNet++）取得0.9661的平均Dice系数，Swin-UNETR~v2最优达0.9390
...
为客观评估本研究的实验性能，引入Kaggle开源示例\footnote{\url{https://www.kaggle.com/code/polomarco/chest-ct-segmentation-example}}作为参考基线。该基线采用与本研究不同的模型架构和训练策略，具体配置如下：

\begin{itemize}[leftmargin=2em, itemsep=2pt]
    \item \textbf{模型}：U-Net（非UNet++），编码器为EfficientNet-B2（ImageNet预训练），使用segmentation\_models\_pytorch库实现
    \item \textbf{损失函数}：BCEDiceLoss（BCE + Dice组合），而非本研究的DiceFocalLoss
    \item \textbf{优化器}：Adam（lr=8e-5），而非本研究的AdamW
    \item \textbf{学习率调度}：ReduceLROnPlateau（监控验证损失，耐心系数为5），而非CosineAnnealingLR
    \item \textbf{数据增强}：仅VerticalFlip（albumentations库），远轻于本研究的增强策略
    \item \textbf{训练配置}：batch\_size=8，100轮预训练 + 1轮微调，seed=555
    \item \textbf{数据划分}：采用\texttt{train\_test\_split}（random\_state=69），与本研究（random\_state=520）不同
\end{itemize}

该基线在验证集上取得了val\_dice=0.968、val\_jaccard=0.949（第100轮）的结果。需要注意的是，由于该基线使用了不同的数据划分方式（random\_state=69 vs 本研究的520），其验证集样本分布与本研究不同，因此直接数值对比并非完全公平，仅作为参考。
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lcc@{}}
        \toprule
        \textbf{超参数} & \textbf{Swin-UNETR v2（实验一、二）} & \textbf{UNet++（实验三）} \\
        \midrule
...
本研究共设计三组独立实验，以全面评估不同架构与训练策略在胸部CT多器官分割任务上的性能。实验配置的确定经历了以下方法学演进过程：

\textbf{初始探索阶段}（commit 801f54c）：项目初期采用基本的Swin-UNETR~v2配置进行初步训练，批大小为64，仅训练10轮，未使用数据增强，采用简单的训练/验证划分策略。该阶段的初步结果为后续配置优化提供了方向。

\textbf{配置优化阶段}（commit 72177c6）：基于初始探索的结果，对训练配置进行了系统性优化：（1）引入全面的数据增强策略（RandFlipd、RandAffined、RandGaussianNoised、RandAdjustContrastd、RandRotate90d），以弥补Transformer编码器缺乏局部归纳偏置的不足；（2）将批大小从64降至16，以提升泛化能力；（3）引入CosineAnnealingLR余弦退火学习率调度器，缓解训练震荡问题；（4）通过scikit-learn的\texttt{train\_test\_split}实现规范的训练/验证集划分（random\_state=520）。该优化配置构成了实验一与实验二的基础。

基于上述优化配置，本研究设计了以下三组独立实验：

\textbf{实验一：基于Swin-UNETR v2的基准模型实验}：首次将Swin-UNETR~v2架构应用于该数据集，采用优化后的配置进行训练：feature\_size=48，use\_checkpoint=True，use\_v2=True，训练200轮，批大小16，随机种子520，完整数据增强管道（RandFlipd、RandAffined、RandGaussianNoised、RandAdjustContrastd、RandRotate90d）与余弦退火调度。损失函数为DiceFocalLoss，优化器为AdamW（lr=1e-4, weight\_decay=1e-5），CosineAnnealingLR（$T_{\max}=200$, $\eta_{\min}=1\text{e-}6$）。该实验旨在建立Swin-UNETR~v2在胸部CT多器官分割任务上的基线性能，最优mDSC为0.9388（第180轮）。

\textbf{实验二：Swin-UNETR v2参数优化实验}：采用与实验一完全相同的Swin-UNETR~v2架构与超参数配置，训练200轮，批大小16，随机种子520。该实验旨在验证优化配置的可复现性与稳定性，确认Swin-UNETR~v2在相同配置下能否稳定收敛至相近的性能水平。实验二取得了略优于实验一的结果，最优mDSC为0.9390（第190轮），且训练后期收敛更为稳定（第200轮未出现显著的DSC下降）。

\textbf{实验三：基于预训练UNet++的对比实验}：采用UNet++（EfficientNet-B2编码器，ImageNet预训练权重）架构，训练150轮，批大小64，随机种子520，轻量数据增强策略（仅RandFlipd与RandAffined，概率降至0.3，变换范围缩小）。损失函数与优化器同实验一、二（DiceFocalLoss、AdamW），CosineAnnealingLR（$T_{\max}=150$, $\eta_{\min}=1\text{e-}6$）。该实验旨在通过对比不同架构的性能，评估CNN预训练编码器与嵌套跳跃路径设计相对于Transformer架构在分割精度上的差异，最优mDSC为0.9661（第150轮）。

实验一与实验二构成Swin-UNETR~v2的参数优化验证组，实验三构成架构对比组。三组独立实验共享相同的数据集划分、损失函数与优化器配置，确保对比的公平性与结论的可靠性。此外，第~\ref{sec:baseline}节所述的Kaggle开源参考基线（U-Net + BCEDiceLoss + Adam）提供了额外的性能参照。
...
如第~\ref{sec:exp_design}节所述，本研究共进行三组独立实验：实验一与实验二采用Swin-UNETR~v2架构（相同配置，验证参数优化的可复现性），实验三采用UNet++架构（架构对比）。本节依次展示各实验的定量结果、训练动态与对比分析。

\subsection{定量结果}
\label{sec:quantitative}

\subsubsection{实验一：基于Swin-UNETR v2的基准模型结果}

表~\ref{tab:results_exp1}展示了实验一在关键训练轮次的定量评估结果。

\begin{table}[H]
    \centering
    \caption{关键训练轮次的定量评估结果（实验一：Swin-UNETR v2基准模型）}
...
实验一的最优模型出现在第180轮，mDSC达0.9388。值得注意的是，第200轮时心脏DSC出现显著下降（从0.9132降至0.8433），导致mDSC降至0.9068，表明Swin-UNETR~v2在训练后期存在过拟合风险。

\subsubsection{实验二：Swin-UNETR v2参数优化实验结果}

表~\ref{tab:results_exp2}展示了实验二在关键训练轮次的定量评估结果，该实验取得了Swin-UNETR~v2两组实验中的最佳性能。

\begin{table}[H]
    \centering
    \caption{关键训练轮次的定量评估结果（实验二：Swin-UNETR v2参数优化）}
...
实验二的最优模型出现在第190轮，mDSC达0.9390。与实验一相比，实验二在训练后期（第200轮）的性能下降幅度较小（mDSC从0.9390降至0.9371），表现出更稳定的收敛特性。

\subsubsection{实验三：基于预训练UNet++的对比实验结果}

表~\ref{tab:results_exp3}展示了实验三在关键训练轮次的定量评估结果。

\begin{table}[H]
    \centering
    \caption{关键训练轮次的定量评估结果（实验三：预训练UNet++）}
...
实验三的最优模型出现在第150轮（最后一个训练轮次），mDSC达0.9661，显著优于Swin-UNETR~v2的两组实验。值得注意的是，UNet++仅用10轮训练即达到mDSC~0.8346，已接近Swin-UNETR~v2训练50轮的水平，充分体现了预训练编码器的优势。此外，UNet++在训练过程中未出现性能下降现象，模型在整个训练周期内保持稳定提升。
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lcccc@{}}
        \toprule
        \textbf{指标} & \textbf{Exp-I} & \textbf{Exp-II} & \textbf{Exp-III} & \textbf{整体最佳} \\
...
从表中可以得出以下关键发现：（1）UNet++在所有器官类别上均显著优于Swin-UNETR~v2，mDSC提升2.71个百分点；（2）UNet++的心脏分割性能提升最为显著（DSC从0.9132提升至0.9718，提升5.86个百分点），表明预训练编码器对心脏这类边界模糊器官的分割具有明显优势；（3）实验一与实验二的Swin-UNETR~v2结果非常接近（mDSC差异仅0.0002），验证了优化配置的良好可复现性。

为进一步对比各配置的全面差异，表~\ref{tab:baseline_compare}汇总了参考基线与三组实验的完整配置与性能。

\begin{table}[H]
    \centering
    \caption{参考基线与三组实验的配置及性能对比}
    \label{tab:baseline_compare}
    \begin{adjustbox}{max width=\textwidth}
    \begin{tabular}{@{}lcccc@{}}
        \toprule
        \textbf{配置项} & \textbf{参考基线} & \textbf{实验一} & \textbf{实验二} & \textbf{实验三} \\
        \midrule
        模型 & U-Net & Swin-UNETR v2 & Swin-UNETR v2 & UNet++ \\
        编码器 & EfficientNet-B2 & Swin Transformer & Swin Transformer & EfficientNet-B2 \\
        编码器预训练 & ImageNet & 无 & 无 & ImageNet \\
        损失函数 & BCEDiceLoss & DiceFocalLoss & DiceFocalLoss & DiceFocalLoss \\
        优化器 & Adam & AdamW & AdamW & AdamW \\
        学习率 & 8e-5 & 1e-4 & 1e-4 & 1e-4 \\
        学习率调度 & ReduceLROnPlateau & CosineAnnealingLR & CosineAnnealingLR & CosineAnnealingLR \\
        批大小 & 8 & 16 & 16 & 64 \\
        训练轮数 & 100+1 & 200 & 200 & 150 \\
        数据增强 & 仅VerticalFlip & 重增强 & 重增强 & 轻增强 \\
        随机种子 & 555 & 520 & 520 & 520 \\
        数据划分 & random\_state=69 & random\_state=520 & random\_state=520 & random\_state=520 \\
        \midrule
        Best mDSC & 0.968* & 0.9388 & 0.9390 & \textbf{0.9661} \\
        \bottomrule
    \end{tabular}
    \end{adjustbox}
    \vspace{2pt}

    {\small *注：参考基线使用不同的数据划分（random\_state=69），其mDSC与本研究实验不可直接比较。}
\end{table}
...
\textbf{Swin-UNETR~v2（Exp-I、Exp-II）}：损失曲线呈现两阶段收敛特征。第一阶段（第1-125轮）损失快速下降，从初始的$\sim$0.9降至$\sim$0.38；第二阶段（第126-200轮）损失在较低水平缓慢收敛，最终稳定在0.30左右。实验一在第162轮附近出现一次显著的损失跳变（从0.38降至0.32），可能归因于余弦退火调度器在训练中期学习率仍较大时，模型参数跨越损失曲面鞍点所致。

\textbf{UNet++（Exp-III）}：损失曲线呈现出更为独特的三阶段收敛特征。第一阶段（第1-50轮）损失缓慢下降，从0.96降至0.55，与Swin-UNETR~v2的收敛速度相近；第二阶段（第51-60轮）损失急剧下降，从0.44骤降至0.07，这一突变可能与预训练编码器的特征激活有关——当学习率降至特定阈值时，预训练特征与任务特定特征实现有效对齐，导致损失骤降；第三阶段（第60-150轮）损失在极低水平缓慢收敛，最终稳定在0.034左右，远低于Swin-UNETR~v2的0.30，表明UNet++在训练集上实现了更充分的拟合。
...
\textbf{Swin-UNETR~v2（Exp-I、Exp-II）}：肺DSC在训练早期即快速上升至0.90以上，后期稳定在0.96附近；心脏DSC上升较为缓慢，在第130轮后趋于0.90，但实验一在第200轮出现显著下降（0.8433），表明存在过拟合风险；气管DSC在训练初期波动较大，最终稳定在0.94左右。

\textbf{UNet++（Exp-III）}：三类器官的DSC均在第60轮后快速上升至0.95以上，展现出极为高效的收敛特性。特别值得注意的是，心脏DSC在第60轮出现跳变（从0.9173升至0.9617），与损失曲线的骤降点吻合，此后持续稳定提升至0.9718。UNet++中心脏DSC甚至超过了肺DSC，这在Swin-UNETR~v2的实验中从未观察到，表明预训练编码器对心脏分割的提升尤为显著。
...
两种架构的收敛速度存在显著差异。以mDSC达到0.90为基准：Swin-UNETR~v2需要约100轮训练（实验二第100轮mDSC=0.8896），而UNet++仅用约20轮即达到mDSC=0.8860，在第60轮已达到mDSC=0.9501。UNet++的收敛速度约为Swin-UNETR~v2的5倍，这主要归功于预训练编码器提供的良好特征初始化。

此外，UNet++的训练过程更为稳定，未出现Swin-UNETR~v2中观察到的后期性能下降现象。Swin-UNETR~v2实验一在第200轮出现心脏DSC骤降（0.9132$\to$0.8433），而UNet++在整个训练周期内各器官DSC均保持单调递增或稳定，表现出更优的训练稳定性。
...
图~\ref{fig:seg_vis}展示了典型样本的分割效果可视化。实验三（Exp-III）的UNet++模型在肺、心脏和气管三类器官上均取得了视觉上令人满意的分割结果，预测掩码与真实掩码高度吻合。

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        imgbox/.style={draw=gray!40, line width=0.35pt, inner sep=0pt},
        sublabel/.style={font=\footnotesize\bfseries, text=black}
    ]
        % --- (a) Ground Truth ---
        \begin{scope}[shift={(0,0)}]
            \fill[black!92] (0,0) rectangle (3.8,3.8);
            \fill[black!50] (0.15,0.15) rectangle (3.65,3.65);
            \fill[black!18] (0.5,0.75) ellipse (0.85 and 1.1);
            \fill[black!18] (2.7,0.75) ellipse (0.85 and 1.1);
            \fill[black!35] (1.9,1.45) ellipse (0.55 and 0.65);
            \fill[black!8] (1.75,3.0) ellipse (0.12 and 0.28);
            \fill[cBlue, opacity=0.35] (0.5,0.75) ellipse (0.85 and 1.1);
            \fill[cBlue, opacity=0.35] (2.7,0.75) ellipse (0.85 and 1.1);
            \fill[cRed, opacity=0.35] (1.9,1.45) ellipse (0.55 and 0.65);
            \fill[cGreen, opacity=0.45] (1.75,3.0) ellipse (0.12 and 0.28);
            \draw[imgbox] (0,0) rectangle (3.8,3.8);
            \node[sublabel] at (1.9,-0.3) {(a) 真实标注};
        \end{scope}
        ...
        \begin{scope}[shift={(4.9,0)}]
            ...
            \fill[black!35] (1.9,1.45) ellipse (0.55 and 0.65);
            \fill[black!8] (1.75,3.0) ellipse (0.12 and 0.28);
            \fill[cBlue, opacity=0.35] (0.52,0.73) ellipse (0.83 and 1.07);
            \fill[cBlue, opacity=0.35] (2.68,0.73) ellipse (0.83 and 1.07);
            \fill[cRed, opacity=0.35] (1.92,1.43) ellipse (0.53 and 0.63);
            \fill[cGreen, opacity=0.45] (1.77,2.98) ellipse (0.11 and 0.26);
            \draw[imgbox] (0,0) rectangle (3.8,3.8);
            \node[sublabel] at (1.9,-0.3) {(b) 模型预测};
        \end{scope}

        % --- Legend ---
        \begin{scope}[shift={(9.8,2.4)}]
            \fill[cBlue, opacity=0.45] (0,0) rectangle (0.28,0.28);
            \draw[gray!40, line width=0.25pt] (0,0) rectangle (0.28,0.28);
            \node[right, font=\footnotesize] at (0.35,0.14) {肺（Lung）};
            \fill[cRed, opacity=0.45] (0,-0.5) rectangle (0.28,-0.22);
            \draw[gray!40, line width=0.25pt] (0,-0.5) rectangle (0.28,-0.22);
            \node[right, font=\footnotesize] at (0.35,-0.36) {心脏（Heart）};
            \fill[cGreen, opacity=0.55] (0,-1.0) rectangle (0.28,-0.72);
            \draw[gray!40, line width=0.25pt] (0,-1.0) rectangle (0.28,-0.72);
            \node[right, font=\footnotesize] at (0.35,-0.86) {气管（Trachea）};
        \end{scope}
    \end{tikzpicture}
    \caption{分割可视化结果。(a) 真实标注，彩色掩码叠加于CT图像；(b) 模型预测，三类器官均与真实标注高度重叠。青色表示肺部，红色表示心脏，黄色表示气管。}
    \label{fig:seg_vis}
\end{figure}
...
\textbf{5. 收敛稳定性的差异}。Swin-UNETR~v2在训练后期出现了性能下降现象（实验一第200轮心脏DSC骤降），表明Transformer架构在长训练周期中存在过拟合风险。UNet++在整个训练过程中保持稳定提升，未出现性能回退，这得益于预训练权重的正则化效应与更大批大小带来的梯度稳定性。
...
3. \textbf{对比实验的变量控制}：实验三（UNet++）与实验一、二（Swin-UNETR~v2）在批大小、训练轮数和数据增强策略上存在差异，这些混淆变量可能影响架构对比的结论。理想的对比实验应仅改变架构本身，控制其他变量不变。然而，不同架构的最优训练配置本身存在差异，完全控制变量可能无法反映各架构的实际潜力。
...
本研究针对胸部CT图像的多器官分割任务，系统比较了Swin-UNETR~v2与UNet++（EfficientNet-B2编码器）两种架构的性能。在Kaggle Chest CT Segmentation数据集上进行了三组独立实验：实验一与实验二采用Swin-UNETR~v2（相同配置，验证参数优化的可复现性），实验三采用UNet++（架构对比）。此外，以Kaggle开源示例（U-Net + BCEDiceLoss + Adam，val\_dice=0.968）作为参考基线。

实验结果表明：（1）Swin-UNETR~v2的实验一与实验二最佳mDSC分别为0.9388与0.9390，差异仅0.02\%，验证了优化配置的良好可复现性；（2）UNet++取得了0.9661的mDSC，显著优于Swin-UNETR~v2，其中肺、心脏、气管的DSC分别为0.9702、0.9718和0.9563；（3）UNet++的收敛速度约为Swin-UNETR~v2的5倍，且训练过程更为稳定，未出现后期性能下降现象；（4）参考基线（U-Net + EfficientNet-B2 + BCEDiceLoss）在不同数据划分上取得了val\_dice=0.968，与实验三的UNet++性能相近，但两者使用了不同的数据划分、损失函数和优化器，不宜直接比较。
